...
def test_rtsp_decode_bind_vps_bind_disp(rtsp_url):
    ...
    while True:
        ...
        # First frames fed to the decoder should include PPS/SPS, or the decoder may exit with "FAILED TO DEC_PIC_HDR"
        if (nalu_types[0] in [1, 5]) and find_pps_sps == 0:
            continue

        find_pps_sps = 1
        if stream_frame is not None:
            ret = dec.set_img(stream_frame.tobytes(), 0)  # Send bitstream; decode several frames before pulling output
            if ret != 0:
                return ret
            if skip_count < 5:
                skip_count += 1
