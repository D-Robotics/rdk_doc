...
CAN驱动支持接收多包合并数据传输功能，当CAN驱动接收到一定阈值的数据包时，会将它们合并，通过IPC从MCU传输到Acore。目的是为了减少IPC传输频率，提高传输效率。在此基础上提供了合包数量和超时时间两个可配参数。
1. 合包数量：用于设置CAN驱动接收到多少包之后，进行一次IPC传输， 默认为1。
...
```
acore侧也支持多包合并传输功能，下面只简单介绍相关数据结构，具体可以使用[应用sample](#应用sample)中提供的测试用例进行修改验证。
```c
