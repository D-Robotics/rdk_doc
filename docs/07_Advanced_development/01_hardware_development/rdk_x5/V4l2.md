...
HBN框架覆盖了 Camera Sensor 采集链路中大部分的模块，包括 `VIN`、 `ISP`、 `VSE`、 `GDC`、 `N2D`。其中`VIN`、 `ISP`、 `VSE`可以通过V4L2接口访问。
...
`VIN` 用于图形采集，可以从4路`SIF`接口中采集到原生图形，一般为RAW格式；
...
:::info 注意
多媒体示例会根据`srpi-config`配置自动选择摄像头，请确保摄像头配置正确。如果`cam0` `cam1`都配置，则默认选择`cam1`
:::
