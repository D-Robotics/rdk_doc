...
本文档为D-Robotics RDK套件的用户手册，为开发者提供关于RDK X3（旭日X3派）、RDK X3 Module（旭日X3模组）、RDK Ultra等产品的使用说明和开发指导，内容涵盖硬件设计、系统定制、应用开发、算法工具链等多个方面。欢迎用户更新体验，具体方法请参考 [系统烧录](./01_Quick_start/install_os.md) 章节。
...
用户如需确认系统版本号，可通过该命令查询 `cat /etc/version`, 2.1.0及以上版本可以通过 `rdkos_info` 命令查看具体的版本信息。
:::
...
**三、基础应用开发**  
介绍系统中预装的功能示例，如io管脚控制、音视频采集，多媒体初步使用等。  
