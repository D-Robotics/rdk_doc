...
RDK 套件目前提供 Ubuntu 22.04 系统镜像，有 desktop，server 两个版本可供选择；
...
XBurn 是官方提供的镜像烧录工具，可以用于烧录 RDK X5 NAND 固件。它提供了直观的图形界面，用户只需点击几步即可完成镜像烧录，非常便捷。
...
macOS 下载 `XBurn-gui_版本号_x64-setup.dmg`。
