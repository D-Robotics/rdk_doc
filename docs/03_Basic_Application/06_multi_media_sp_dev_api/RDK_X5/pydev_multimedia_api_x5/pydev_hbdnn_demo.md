...
- 通过本地 DEB 包安装
  ...
  ```
t-get install hobot-spdev
  ```

- 常见问题
  - 如果 .deb 安装后文件未生效，检查是否有其他依赖阻止其覆盖（如已有老版本 hobot-spdev）。
  - 可使用 dpkg -L hobot-spdev 查看文件是否成功部署。
...
### 环境准备
  请确保已正确安装 HBMRuntime（详见[安装说明](#安装说明installation)），并已具备模型文件 bin 模型。
### 示例
#### 单模型单输入推理
  适用于模型只有一个输入张量的情况。
```python
...
- hbm_descs: Dict[str, str]
  - 功能说明：
    - 每个 HBM 文件中的备注信息。
  - 结构说明：
    - Dict[str, int]：键为.bin 文件名（例如 "resnet18"），值为HBM 文件中的注释或元信息字符串。
  - 示例：
    ```python
    #打印所有模型文件的描述信息
    print(model.hbm_descs)
    # 输出：{'/opt/hobot/model/s100/basic/yolov5x_672x672_nv12.bin': 'xxx'}
    ```
...
- input_strides: Dict[str, Dict[str, List[int]]]
  ...
  - 示例：
    ...
    ```
    注意： stride 的详细含义可参考[OE文档](https://developer.d-robotics.cc/api/v1/fileData/horizon_j5_open_explorer_cn_doc/runtime/source/bpu_sdk_api/source/bpu_sdk_api_doc.html?highlight=stride#hbdnntensorproperties)中 hbDNNTensorProperties 中的描述；
